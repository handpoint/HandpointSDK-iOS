...
Frame::Frame(const uint8_t* pData, int len, bool partial)
    : write_data(ciMinSize + len), bPartial(partial), m_pData(0), m_len(0)
{
	...
	uint16_t etx = bPartial ? FRAME_PARTIAL_END : FRAME_END;
...
void Frame::AddCRC(){
	FramePayload* pFrame = GetPayload<FramePayload>();
	uint16_t crc = htons(CRC::CalcCRC(&pFrame->iCommand,
                                       write_data.size() - sizeof(pFrame->StartSequence) - sizeof(crc)
                                      )
                         );
	uint8_t* pCrc = &write_data[write_data.size() - sizeof crc];
...
bool Frame::isValidCrc(){
	...
	uint16_t crc = htons(CRC::CalcCRC(&pFrame->iCommand,
                                      m_len - sizeof(pFrame->StartSequence) - sizeof(crc))
                         );
