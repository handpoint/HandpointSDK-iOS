...
@interface iOSConnection () <NSStreamDelegate>
@end

@implementation iOSConnection
{
...
{
    LOG(@"%@", ::dump(@"iOSConnection::WriteData : ", data, (int) len));
...
{
    LOG(@"iOSConnection::write_from_queue_to_stream");
    @synchronized (outputData)
    {
        if ([outputData length] > 0)
        {
            ...
            LOG(@"iOSConnection::write_from_queue_to_stream, sent %d bytes, len=%d", (int) written, (int) [outputData length]);
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode
{
    ...
    switch (eventCode)
    {
        case NSStreamEventOpenCompleted:
            LOG(@"iOSConnection::handleEvent, NSStreamEventOpenCompleted");
            break;
        case NSStreamEventHasBytesAvailable:
            LOG(@"iOSConnection::handleEvent, NSStreamEventHasBytesAvailable");
            ...
            if (aStream == inputStream)
            {
                ...
                do
                {
                    ...
                    LOG(@"%@ (%d bytes)", ::dump(@"iOSConnection::handleEvent: ", buf, (int) nread), (int) nread);
            ...
            {
                LOG(@"iOSConnection::handleEvent, stream is not an inputStream");
            }
        ...
        case NSStreamEventEndEncountered:
            LOG(@"iOSConnection::handleEvent, NSStreamEventEndEncountered");
            [self shutdown];
            break;
        case NSStreamEventErrorOccurred:
            LOG(@"iOSConnection::handleEvent, NSStreamEventErrorOccurred");
            [self shutdown];
            break;
        case NSStreamEventHasSpaceAvailable:
            if (aStream == outputStream)
            {
                LOG(@"iOSConnection::handleEvent, NSStreamEventHasSpaceAvailable on outputStream");
                [self write_from_queue_to_stream];
            }
            else
            {
                LOG(@"iOSConnection::handleEvent, NSStreamEventHasSpaceAvailable on inputStream!");
            }
            break;

        default:
            LOG(@"iOSConnection::handleEvent, unhandled event");
            break;
    }
